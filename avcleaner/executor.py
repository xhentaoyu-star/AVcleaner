...
def _sync_quarantine_restore_status(run_id: str, item_id: str, status: str) -> bool:
    try:
        update_quarantine_manifest_restore_status(run_id, item_id, status)
    except Exception:
        return False
    return True
        ...
        if preview_item["blocking"]:
            ...
            try:
                ...
                rollback_item = rollback_item.model_copy(
                    update={
                        "state": RunItemState.ROLLBACK_FAILED,
                        "message": error_code if manifest_status_synced else f"{error_code};manifest_status_update_failed",
                        "issue_code": error_code,
                        "rollback_status": str(RunItemState.ROLLBACK_FAILED),
                        "rollback_error_code": error_code,
                    }
                ...
                rollback_item = rollback_item.model_copy(
                    update={
                        ...
                        "message": "restored" if manifest_status_synced else "restored;manifest_status_update_failed",
